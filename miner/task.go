...
func (task *Task) applyTransactionsAndDebts(scdo SeeleBackend, statedb *state.Statedb, accountStateDB database.Database, parent *types.Block, engine consensus.Engine, log *log.SeeleLog) error {
	now := time.Now()
	// entrance
	...
	if task.header.Consensus == types.BftConsensus {
		...
		txHashStem, stateHashStem, err := task.getStemHashes(scdo, statedb, log)
		if err != nil {
			return err
		}

		// update recentTxHashStem
		recentTxHashStem, err := task.getRecentTxHashStem(scdo, log)
		if err != nil {
			return err
		}
...
func (task *Task) getStemHashes(scdo SeeleBackend, statedb *state.Statedb, log *log.SeeleLog) (common.Hash, common.Hash, error) {
	...
	for txIndex, tx := range task.txs {
		...
		for i := 0; i < 2; i++ {
			...
			if isExisted, err := scdo.GetAccountIndexDB().Has(account.Bytes()); err != nil {
				return common.EmptyHash, common.EmptyHash, err
			} else if !isExisted {
				task.accountCount++
				...
				if err = scdo.GetAccountIndexDB().Put(account.Bytes(), indexBytes); err != nil {
					return common.EmptyHash, common.EmptyHash, err
				}

				if err = scdo.GetIndexAccountDB().Put(indexBytes, account.Bytes()); err != nil {
					return common.EmptyHash, common.EmptyHash, err
				}
			}
		}
	...
	for i := uint64(0); i < task.accountCount; i++ {
		...
		accountBytes, err := scdo.GetIndexAccountDB().Get(indexBytes)
		if err != nil {
			return common.EmptyHash, common.EmptyHash, err
		}
...
func (task *Task) getRecentTxHashStem(scdo SeeleBackend, log *log.SeeleLog) (common.Hash, error) {
	...
	if int(task.header.Height)%int(common.RelayInterval) > 0 {
		...
		for i := start; i <= end; i++ {
			...
			if i != end {
				prevBlock, err := scdo.BlockChain().GetStore().GetBlockByHeight(uint64(i))
				if err != nil {
